...
def redirect2GitHub():
   requestUrl = f"https://github.com/login/oauth/authorize?client_id={clientID}&redirect_uri={redirect_uri}&scope=read:user"
   return flask.redirect(requestUrl, code=301)

@app.route('/login/callback')
def GitHubcallback():
   code = flask.request.args.get('code')
   requestUrl = f"https://github.com/login/oauth/access_token?client_id={clientID}&client_secret={clientSecret}&code={code}"
   headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36 Edg/138.0.0.0",
                "Accept": "application/json"
            }
   x = requests.get(requestUrl, headers=headers)
   access_token = json.loads(x.text)["access_token"]
   encryptedToken = hashlib.md5(code.encode('utf-8')).hexdigest()
   ...
   username = json.loads(y.text)["login"]
   ...
   if results == ():
      ...
      sql = f'UPDATE checktoolusers SET encryptedToken = "{encryptedToken}" WHERE name = "{username}"'
      cursor.execute(sql)
      db.commit()
   closeDB(db)
